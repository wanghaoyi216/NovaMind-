package com.novamind.course.service.impl;
...
import com.novamind.course.constants.CourseConstants;
import com.novamind.course.constants.CourseErrorInfo;
import com.novamind.course.constants.CourseStatus;
import com.novamind.course.domain.dto.CourseBaseInfoSaveDTO;
import com.novamind.course.domain.dto.CoursePageQuery;
import com.novamind.course.domain.po.*;
import com.novamind.course.domain.vo.CourseBaseInfoVO;
import com.novamind.course.domain.vo.CoursePageVO;
import com.novamind.course.domain.vo.CourseSaveVO;
import com.novamind.course.domain.vo.NameExistVO;
import com.novamind.course.mapper.*;
import com.novamind.course.service.*;
    ...
    public CourseBaseInfoVO getCourseBaseInfo(Long id, Boolean see) {
        ...
        if (see) {
            ...
            if (course != null) {
                ...
                // course_content 与 course 不是同一事务写入，历史/外部导入的课程可能没有内容行，
                // 这里必须判空，否则 setDetail(...) 直接 NPE -> 500。
                final String detailText = courseContent == null ? null : courseContent.getCourseDetail();
                final String introduceText = courseContent == null ? null : courseContent.getCourseIntroduce();
                final String usePeopleText = courseContent == null ? null : courseContent.getUsePeople();
                //1.4.设置课程评分
                ...
                //1.9.设置课程详情
                courseBaseInfoVO.setDetail(detailText);
                //1.10.设置课程介绍
                courseBaseInfoVO.setIntroduce(introduceText);
                //1.11.设置课程适用人群
                courseBaseInfoVO.setUsePeople(usePeopleText);
                //1.12.设置小节总数量
