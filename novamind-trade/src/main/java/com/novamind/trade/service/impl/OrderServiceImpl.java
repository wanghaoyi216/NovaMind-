...
import com.novamind.api.constants.CourseStatus;
...
import com.novamind.api.dto.trade.OrderBasicDTO;
...
import com.novamind.trade.constants.RefundStatus;
...
import com.novamind.trade.domain.vo.*;
import com.novamind.trade.mapper.OrderMapper;
import com.novamind.trade.service.ICartService;
...
import static com.novamind.common.constants.ErrorInfo.Msg.OPERATE_FAILED;
import static com.novamind.trade.constants.TradeErrorInfo.ORDER_ALREADY_FINISH;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements IOrderService {
    ...
    /**
     * R23 Cache-Aside 统一封装：替代 @Cacheable 注解，
     * 用于"订单详情读多写少"场景的跨实例缓存。
     */
    private final CacheAsideTemplate cacheAside;

    /**
     * R9 Hot Key 防护：{@code coupon:{userId}} 在下单/查询页面会被反复访问。
     * 用户进入下单页时调用 {@link PromotionClient#findDiscountSolution(List)}，
     * 同用户重复进入会被分桶本地缓存吸收，分散到 16 个独立桶。
     */
    private final HotKeyGuard<String, List<CouponDiscountDTO>> couponDiscountGuard =
            HotKeyGuard.<String, List<CouponDiscountDTO>>builder()
                    .name("coupon")
                    .maxSizePerBucket(2_000)
                    .expireAfterWrite(java.time.Duration.ofSeconds(30))
                    .allowNull(true)
                    .build();

    /**
     * R9 Hot Key 防护：{@code coupon:detail:{couponIds+courseIds}} 是下单接口
     * 的热读路径（用户点"提交订单"会反复打这个 RPC）。这里把 key 拼成
     * sorted(courseId)+sorted(couponId) 的字符串，按 hash 分桶。
     */
    private final HotKeyGuard<String, CouponDiscountDTO> couponDetailGuard =
            HotKeyGuard.<String, CouponDiscountDTO>builder()
                    .name("coupon:detail")
                    .maxSizePerBucket(3_000)
                    .expireAfterWrite(java.time.Duration.ofSeconds(60))
                    .allowNull(false)
                    .build();

    /** 把 {@code coupon:{ids}} 的 key 序列化成稳定字符串（按字典序排序） */
    private static String stableKey(List<Long> ids) {
        if (ids == null || ids.isEmpty()) return "empty";
        // 拷贝再排序，避免修改入参影响后续业务（如 writeOffCoupon）
        java.util.List<Long> sorted = new java.util.ArrayList<>(ids);
        java.util.Collections.sort(sorted);
        return sorted.toString();
    }

    /** 把课程 id 列表序列化成稳定字符串 */
    private static String courseKey(List<OrderCourseDTO> orderCourses) {
        if (orderCourses == null || orderCourses.isEmpty()) return "empty";
        java.util.List<Long> sorted = orderCourses.stream()
    ...
    @Override
    @Transactional
    @Idempotent(key = "#req.orderId", message = "订单重复创建，请勿重复提交")
    public PlaceOrderResultVO placeOrder(PlaceOrderDTO req) {
        Long userId = UserContext.getUser();
        // 1.查询课程费用信息，如果不可购买，这里直接报错
        List<CourseSimpleInfoDTO> courseInfos = getOnShelfCourse(req.getCourseIds());
        // 2.封装订单信息
        ...
        // 删除null的优惠券id，如果存在的话
        CollUtil.removeNull(req.getCouponIds());
        ...
        if (CollUtils.isNotEmpty(couponIds)) {
            List<OrderCourseDTO> orderCourses = courseInfos.stream()
                    ...
                    .collect(Collectors.toList());
            // R9: 热读路径，分桶本地缓存吸收 RPC 调用
            String guardKey = stableKey(couponIds) + "|" + courseKey(orderCourses);
            discount = couponDetailGuard.get(guardKey, () ->
                    promotionClient.queryDiscountDetailByOrder(new OrderCouponDTO(couponIds, orderCourses)));
            if (discount != null) {
        ...
        // 5.删除购物车数据
        cartService.deleteCartByUserAndCourseIds(userId, req.getCourseIds());
    ...
    public OrderConfirmVO prePlaceOrder(List<Long> courseIds) {
        ...
        List<OrderCourseDTO> orderCourses = courseInfos.stream()
                ...
                .collect(Collectors.toList());
        List<CouponDiscountDTO> discountSolution = couponDiscountGuard.get(
                "u:" + UserContext.getUser() + "|" + courseKey(orderCourses),
    ...
    public OrderVO queryOrderById(Long id) {
        // R23 案例 2：原本每次都打 DB+RPC（订单详情 + 优惠明细），
        // 改用 CacheAsideTemplate 把 OrderVO 整体缓存 5 分钟，
        // 5 分钟内任何实例读都直接命中 Redis，绕过 MySQL。
        OrderVO vo = cacheAside.get("trade:order:detail:" + id, OrderVO.class,
                () -> loadOrderVOById(id),
    ...
    /**
     * R23 案例 2 配套：实际加载 OrderVO 的逻辑（被 cacheAside.get 当 Loader 调用）。
     */
    private OrderVO loadOrderVOById(Long id) {
        // 1.查询订单
        Order order = getById(id);
        if (order == null) {
            return null;
    ...
    /**
     * R23 案例 2 配套：写路径删缓存。
     * 取消订单 / 删除订单 / 更新订单状态后，调用此方法让缓存失效。
     */
    private void evictOrderCache(Long id) {
        if (id == null) return;
        cacheAside.invalidate("trade:order:detail:" + id);
    }
