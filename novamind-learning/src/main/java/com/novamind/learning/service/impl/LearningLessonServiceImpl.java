...
import com.novamind.api.client.course.CatalogueClient;
import com.novamind.api.client.course.CourseClient;
import com.novamind.api.dto.IdAndNumDTO;
import com.novamind.api.dto.course.CataSimpleInfoDTO;
import com.novamind.api.dto.course.CourseFullInfoDTO;
...
import com.novamind.common.domain.query.PageQuery;
import com.novamind.common.exceptions.BadRequestException;
import com.novamind.common.utils.*;
import com.novamind.learning.domain.po.LearningLesson;
import com.novamind.learning.domain.po.LearningRecord;
import com.novamind.learning.domain.po.PointsRecord;
import com.novamind.learning.domain.vo.LearningLessonVO;
import com.novamind.learning.domain.vo.LearningPlanPageVO;
import com.novamind.learning.domain.vo.LearningPlanVO;
import com.novamind.learning.enums.LessonStatus;
import com.novamind.learning.enums.PlanStatus;
import com.novamind.learning.mapper.LearningLessonMapper;
import com.novamind.learning.mapper.LearningRecordMapper;
import com.novamind.learning.mapper.PointsRecordMapper;
import com.novamind.learning.service.ILearningLessonService;
...
public class LearningLessonServiceImpl extends ServiceImpl<LearningLessonMapper, LearningLesson> implements ILearningLessonService {
     ...
     * queryByUserAndCourseId 是课表查询的高频入口（每次打开"我的课表"都会调），
     * 改用 CacheAsideTemplate 跨实例共享缓存 5 分钟。
     */
    ...
    public void deleteCourseFromLesson(Long userId, Long courseId) {
        ...
        // R23: 写 DB 成功后删缓存
        evictLessonCache(userId, courseId);
    }
    ...
    public LearningLesson queryByUserAndCourseId(Long userId, Long courseId) {
        // R23 案例 3：用 CacheAsideTemplate 替代 @Cacheable。
        // key = "learning:lesson:user:{userId}:course:{courseId}"，
        // TTL = 5 分钟（带 ±10% 抖动防雪崩），Loader 内部回源 DB。
        if (userId == null || courseId == null) {
            return getOne(buildUserIdAndCourseIdWrapper(userId, courseId));
        }
        return cacheAside.get(
                "learning:lesson:user:" + userId + ":course:" + courseId,
                LearningLesson.class,
                () -> getOne(buildUserIdAndCourseIdWrapper(userId, courseId)),
                java.time.Duration.ofMinutes(5));
    }

    /**
     * R23 案例 3 配套：删除课表后必须把缓存也清掉，否则下次 query 仍会拿到旧数据。
     */
    private void evictLessonCache(Long userId, Long courseId) {
        if (userId == null || courseId == null) return;
        cacheAside.invalidate("learning:lesson:user:" + userId + ":course:" + courseId);
    }
    ...
    public void createLearningPlan(Long courseId, Integer freq) {
        ...
        updateById(l);
        // R23: 写 DB 成功后删缓存（weekFreq / planStatus 变了）
        evictLessonCache(userId, courseId);
    }
    ...
    public LearningPlanPageVO queryMyPlans(PageQuery query) {
        ...
        QueryWrapper<PointsRecord> wrapper = new QueryWrapper<PointsRecord>()
                ...
                .lt("create_time", end);
        // 查询本周积分
        Integer score = Objects
                .requireNonNullElse(pointsRecordMapper
                        .queryUserPointsByTypeAndDate(wrapper), 0);
        result.setWeekPoints(score);
        // 4.查询分页数据
